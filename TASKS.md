...
| ID | チケット | ステータス | 担当モデル | 受け入れ条件（要約） |
|---|---|---|---|---|
| V2.1 | PC タブ音声の取込（getDisplayMedia・ヘッドホン会議対応） | DONE（要人間の実ブラウザ確認） | Sonnet5 | マイク/タブ音声切替UI / 共有停止で安全に停止 / 再接続はストリーム再利用 |
| V2.2 | 表示設定（フォントサイズ・英語併記ON/OFF） | BLOCKED | Haiku可 | localStorage 使用の人間承認待ち。承認まで非永続実装のみ可 |
| V2.3 | PWA 化（manifest・アイコン・スタンドアロン表示） | DONE | Haiku可 | ホーム画面から1タップ起動 |
...
| B-1 | 【バグ】無音自動停止が発火しない（V1.5欠陥・effect churn でタイマー消滅） | DONE | Sonnet5 | 無音10分で確実に停止 / 揺れで計測が途切れない |
| B-2 | 【バグ】タブ共有ストリームが早期失敗時にリーク（V2.1レビューで発見） | DONE | Fable5 | 接続前失敗でも共有トラックを解放（回帰テスト有） |
| V2.6 | E2E テスト（Playwright・偽造セッションで認証済みフローも） | DONE | Sonnet5 | E2E 7/7 green（未認証3・PWA3・認証済み配線1）`npm run test:e2e` |
