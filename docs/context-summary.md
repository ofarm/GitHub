...
- **V1.1-V1.6 の 6 つのチケットのうち 5 つが完了** ✅（V1.1/V1.2/V1.3/V1.5/V1.6）
...
- 残る 1 つ:
  - **V1.4: 長時間セッション検証**（60 分稼働テスト・実会議必須・人間確認待ち）。V1.3 の再接続機構に依存（セッション上限に達した場合の張り直しに利用）。
...
## 次のチケット（最大3件）
1. **V1.4 長時間セッション検証**（人間の実会議60分テストが必須。コード側の張り直し対応は V1.3 の再接続機構を流用できるか要確認）。
2. Phase 2 へ進む（V2.1 PC タブ音声取込＝ヘッドホン会議対応、中〜高モデル）。
3. V2.3 PWA 化（Haiku可）。
