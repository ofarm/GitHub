...
- 出してよい（限定）: client secret **発行段階**の上流(OpenAI)エラーの status / error.code / error.type / error.message。この段階では音声・transcript・翻訳本文は一切存在せず、内容は「リクエスト形（モデル名・パラメータ名）」に限られるため診断ログに出してよい。**音声送信開始後の delta 等は対象外（出さない）**。
...
### 依存追加の記録
- `@playwright/test`（devDependency・2026-07-14）: V2.6 E2E テスト用。オーナーの「E2Eテストも進めてください」指示（2026-07-14）を承認とみなして追加。**本番ランタイムには含まれない**（dev のみ）。バージョンは実行環境プリインストールの Chromium (rev 1194) に一致する 1.56.1 に固定。E2E は実キー・実メールを使わず、架空のテスト値のみで動作する（`e2e/testEnv.ts`）。
